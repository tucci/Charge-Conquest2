...
public class helicopterScript : MonoBehaviour
{
    ...
    [SerializeField] private float helicopterRotation;
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(this.transform.position, dir2, out hit2))
        {
            if (hit2.transform.gameObject.CompareTag("Player"))
            {
                ...
                gameObject.transform.rotation = Quaternion.RotateTowards(gameObject.transform.rotation, neededRotation2, Time.deltaTime * helicopterRotation);
        ...
        if (Physics.Raycast(this.transform.position, dir, out hit, 25.0f))
        {
        ...
        if (!playerSeen)
        {
            ...
            gameObject.transform.rotation = Quaternion.RotateTowards(gameObject.transform.rotation, neededRotation, Time.deltaTime * helicopterRotation);



            if ((Quaternion.Angle(transform.rotation, neededRotation)) <= 5.0f)
            {
                transform.position = Vector3.MoveTowards(transform.position, path[tankPathIndex].transform.position, Time.deltaTime * helicopterSpeed);
            }
